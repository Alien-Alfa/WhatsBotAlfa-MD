...
process.on('message', (message) => {
  if (message === 'stop_whatsapp') {
    ...
    console.log("🛑 WhatsApp connection and message processing stopped");
    ...
    console.log("🚀 WhatsApp connection and message processing started");
    // Notify parent process about status change
    if (process.send) {
...
const connect = async () => {
  ...
  const connectionMonitor = setInterval(() => {
    if (connectionStopped && currentConnection) {
      console.log("🛑 Connection monitor: Closing connection due to stop request");
      try {
        currentConnection.end();
        currentConnection = null;
      } catch (error) {
        console.warn("Error in connection monitor cleanup:", error);
      }
...
const handleConnectionUpdate = (conn, ws) => async (s) => {
  ...
  try {
    if (qr) {
      ...
      if (connectionStopped) {
        console.log("🛑 QR generation skipped - connection is stopped");
        return;
      }
      ...
      if (connectionStopped) {
        console.log("🛑 Connection open ignored - connection is stopped");
        conn.end();
        return;
      }
...
const handleMessages = (conn, ws) => async (m) => {
  try {
    ...
    if (!pausedChatsCache || now > pausedCacheExpiry) {
      ...
      console.log("🔍 Updated paused chats cache:", pausedChatsCache.length, "chats");
      if (pausedChatsCache.length > 0) {
        console.log("🔍 Paused chats:", pausedChatsCache.map(chat => chat.jid || chat.chatId));
      }
    }
    ...
    if (connectionStopped) {
      if (isResume) {
        console.log("� Processing resume command while bot is stopped");
        // Allow resume to continue processing
      } else if (isRestart && msg.sudo) {
        console.log("🔄 Processing admin restart command while bot is stopped");
        // Allow admin restart to continue processing
      } else {
        console.log("🛑 Message processing blocked - bot is stopped");
        return;
      }
    }

    if (pausedChatsCache.some((pausedChat) => 
      (pausedChat.chatId === chatId || pausedChat.jid === chatId) && !isResume
    )) {
      console.log("⏸️ Message blocked - chat is paused:", chatId);
      return;
    }
...
process.on("SIGINT", async () => {
  ...
  if (currentConnection) {
    try {
      console.log("🔌 Closing WhatsApp connection on exit...");
      currentConnection.end();
      currentConnection = null;
    } catch (error) {
      console.warn("Error closing connection on exit:", error);
    }
  }
