...
async function WriteSession(id) {
  try {
    ...
    // Try fetch
    try {
      const response = await fetch("https://paste.c-net.org/" + id, { timeout: 10000 });
      if (response.ok) {
    ...
    // Try axios with await
    try {
      const response = await axios.get("https://paste.c-net.org/" + id);
      data = response.data;
      console.log('[SESSION ID]:[' + id + "]");
      console.log('[SESSION]:[' + JSON.stringify(data) + "]");
      fs.writeFileSync("./session/creds.json", JSON.stringify(data));
    ...
    // Try axios with promise
    axios.get("https://paste.c-net.org/" + id)
      .then(response => {
        data = response.data;
        console.log('[SESSION ID]:[' + id + "]");
        console.log('[SESSION]:[' + JSON.stringify(data) + "]");
      ...
      .catch(axiosPromiseError => {
        console.error("[AXIOS Promise Error]: " + axiosPromiseError.message);
      });

    // Try request
    request.get("https://paste.c-net.org/" + id, (requestError, response, body) => {
      if (requestError) {
    ...
    // Try got
    try {
      const response = await got("https://paste.c-net.org/" + id);
