...
// Performance optimized functions.js

// Lazy load heavy dependencies for better startup performance
let axios, jidDecode, delay, generateWAMessageFromContent, proto;
let id3, readFile, unlink, fromBuffer, JSZip, acrcloud, path, FormData;
let spawn, fetch, JSDOM, cheerio, commands, config, jsQR, fs, jimp;
let loadMessage, tmpdir, exec, streamBuffers;
...
const initializeDependencies = () => {
  if (!axios) {
    ...
    id3 = require("browser-id3-writer");
    ...
    fs = require("fs");
    jimp = require("jimp");
    ({ loadMessage } = require("../assets/database/StoreDb"));
    ({ tmpdir } = require("os"));
...
const IG_URL_REGEX = /^https?:\/\/(www\.)?(instagram\.com|instagr\.am)\/(p|reel|tv)\/[a-zA-Z0-9_-]+\/?(\?.*)?$/i;
...
async function m3u82Mp4(m3u8Url) {
  ...
  return new Promise((resolve, reject) => {
    ...
    const tempOutputFile = path.join(tmpdir(), `output_${Date.now()}.mp4`);
    const command = `ffmpeg -i "${m3u8Url}" -c copy "${tempOutputFile}"`;
    ...
    ffmpegProcess.stderr.on("data", (data) => {
      ...
      if (timeMatch) {
        ...
        logger.info(`Conversion progress: ${elapsedTime}`);
      }
    });
  });
}
...
async function WriteSession() {
  initializeDependencies();
  try {
    logger.info("Attempting to generate session from environment variables");
    
    if (process.env.SESSION !== undefined) {
      logger.info('Using SESSION from environment variables');
      const data = JSON.parse(process.env.SESSION);
      fs.writeFileSync("./session/creds.json", JSON.stringify(data));
      logger.info("Session extraction from SESSION variable complete");
      return true;
    } else if (process.env.SESSION_ID !== undefined) {
      const id = process.env.SESSION_ID;
      logger.info(`Attempting to fetch session with ID: ${id}`);
      const url = `https://paste.c-net.org/${id}`;
      const response = await fetchWithRetry(url, { timeout: 1000 });

      if (response) {
        const data = await response.json();
        fs.writeFileSync("./session/creds.json", JSON.stringify(data));
        logger.info("Session extraction from SESSION_ID complete");
        return true;
      } else {
        logger.warn("Failed to fetch session data from paste service");
        return false;
      }
    } else {
      logger.info("No SESSION or SESSION_ID environment variables found");
      return false;
    }
  } catch (error) {
    logger.warn("Failed to generate session from environment", { error: error.message });
    return false;
  }
}
...
module.exports = {
    ...
    async function checkAndInitializeGitRepo() {
      ...
      if (!isGitRepo) {
          try {
              ...
              logger.info('Error initializing git repository:', error);
  ...
  try {
      ...
      if (commits.total === 0) {
          await logger.info('_Bot up to date_');
      } else {
          await logger.info('_Started update.._');
          await new Promise((resolve, reject) => {
              exec('git pull origin main', (error, stdout, stderr) => {
                  error ? reject(error) : resolve(stdout);
              });
          });
          await logger.info('_Successfully updated_');
          await logger.info('_Restarting_');
          await process.send("reset");
      }
  } catch(error) {
      logger.info('Error during update:', error);
      await logger.info( '_Update failed_');
  }
    },
...
const logger = require("./logger");
          ...
          if (!creds || !creds.me || !creds.me.id) {
              logger.error("Credentials file or 'me' property not found.");
              return false;
          }
          ...
          logger.error("Error in 'fromMe' function:", error);
          return false;
      }
  },
};
