...
class Message extends Base {
  ...
  async log() {
    logger.debug("Message data:", this.data);
  }
  ...
  try {
    ...
    logger.error('❌ Error setting full profile picture:', error);
    throw error;
  }
}
...
async generateProfilePicture(imageBuffer) {
  try {
    ...
    logger.error('❌ Error generating profile picture:', error);
    throw error;
  }
}
  ...
  async UpdateHeroku(message) {
    ...
    if (commits.total === 0) {
        ...
        try {
            ...
            logger.error('Error getting Heroku app information:', error);
